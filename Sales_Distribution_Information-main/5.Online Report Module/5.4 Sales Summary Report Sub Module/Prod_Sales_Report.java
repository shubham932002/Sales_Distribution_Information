import java.io.*;
import java.sql.*;
public class Prod_Sales_Report
{
    int productid;
    String prodname, pkgunit;
    Connection con;
    ResultSet rs, rs_prod;
    Prod_Sales_Report(int prodid, String start_date, String end_date)
    {
        productid = prodid;
        try{
            Connection_method(start_date, end_date);
            BufferedWriter bw = new BufferedWriter(new FileWriter("Sales2.html"));
            bw.write("<html>");
            bw.write("<body>");
            bw.write("<center><h1>Samsung Electronics Public Limited</h1></center>");     
            bw.write("<center><h1>Product wise Sales Summary Report</h1></center>");
            bw.write("<center><h4>(Products Sales Register)</h4></center>");
    ...
    public static void main(String args[]){
        new Prod_Sales_Report(2, "16-DEC-2022", "31-DEC-2022");
    }
    void Connection_method(String sdate, String edate) {
        try {
            ...
            String qry = "Select s.sales_billno, s.sales_date, s.wholeseller_id, w.wholeseller_name, w.city, s.sales_qty, s.sales_rate, s.gst_in_per, s.add_amount from Products_Sales s, Wholesellers_Master w where s.wholeseller_id = w.wholeseller_id and s.sales_date between '"+ sdate +"' and '"+ edate +"' and s.product_id = "+ productid +" order by s.sales_date";
