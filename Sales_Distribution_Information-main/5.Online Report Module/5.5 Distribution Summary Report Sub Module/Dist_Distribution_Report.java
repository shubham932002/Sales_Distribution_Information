import java.io.*;
import java.sql.*;
public class Dist_Distribution_Report
{
    int distributor;
    String distname, distcity;
    Connection con;
    ResultSet rs, rs_dist;
    Dist_Distribution_Report(int wholeid, String start_date, String end_date)
    {
        distributor = wholeid;
        try{
            Connection_method(start_date, end_date);
            BufferedWriter bw = new BufferedWriter(new FileWriter("Sales5.html"));
            bw.write("<html>");
            bw.write("<body>");
            bw.write("<center><h1>Samsung Electronics Public Limited</h1></center>");     
            bw.write("<center><h1>Distributor wise Product Distribution Summary Report</h1></center>");
            bw.write("<center><h4>(Distributor's Distribution Register)</h4></center>");
            ...
            bw.write("<th>Distribution Quantity</th>");
    ...
    public static void main(String args[]){
        new Dist_Distribution_Report(2, "16-DEC-2022", "31-DEC-2022");
    }
    void Connection_method(String sdate, String edate) {
        try {
            ...
            String qry = "Select s.distribution_billno, s.distribution_date, s.product_id, p.product_name, p.Packaging_unit, s.distribution_qty, s.distribution_rate, s.gst_in_per, s.add_amount from Products_Distribution s, Products_Master p where s.product_id = p.product_id and s.distribution_date between '"+ sdate +"' and '"+ edate +"' and s.distributor_id = "+ distributor +" order by s.distribution_date";
